...
class Gclass:
    ...
    def update(cls, p):
        ...
        for att in cls.att[1:]:
            value = getattr(obj, att)
            command += f' {att} = {cls.conv(obj, att, value)},'
        id = cls.att[0]
    ...
    def set_filter(cls, f_dic={}):
        if f_dic:
            ...
            for att, listf in f_dic.items():
                s1 = set([getattr(obj, code) for obj in lobj
                          if getattr(obj, att) in listf])
                s = s.union(s1)
            if len(s) > 0:
                cls.lst = list(s)
                cls.pos = 0
    ...
    def sqlexe(cls, command):
        ...
        try:
            ...
            tname = cls.__name__
            cur = con.execute(
                f"SELECT name FROM sqlite_master WHERE type='table' AND name='{tname}'")
            table = cur.fetchone()
